...
import { BarChart2, Vault, ShieldCheck, Settings, Droplet } from "lucide-react";
...
export function Navbar() {
  ...
  const renderLink = ({ href, label, icon: Icon }: { href: string; label: string; icon: React.ComponentType<{ className?: string }> }) => (
    <Link
      key={href}
      href={href}
      className={clsx(
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-surface-border bg-surface/80 backdrop-blur-md">
      <div className="container mx-auto px-4 max-w-7xl flex items-center justify-between h-16 gap-3">
        {/* Logo */}
